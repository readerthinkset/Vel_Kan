"""
YouTube Upload Script - Velocity Kannada
Updated for 2025
...
def generate_video_metadata(category: str, num_phrases: int, phrases: list = None):
    """Generate Kannada title, description, and tags for the video"""

    title = f"Kannada Learning: {num_phrases} Essential {category} Phrases"

    description_lines = [
        f"Learn Kannada with Velocity Kannada!",
        f"",
        f"Category: {category}",
        f"",
        f"Master Kannada one phrase at a time! Today's {category} lesson:",
        f""
    ]
    ...
    if phrases:
        ...
        for i, phrase in enumerate(phrases[:5], 0):
            ...
            description_lines.append(f"   {phrase.get('kannada', '')}")
    ...
    description_lines.extend([
        ...
        f"Subscribe for daily Kannada lessons!",
        f"",
        f"Pronunciation Guide:",
        f"   The phonetic spelling in brackets helps you say it correctly!",
        f"",
        f"#LearnKannada #KannadaLessons #KannadaForBeginners #LanguageLearning",
        f"#Kannada #Education #Tutorial #DailyKannada #{category.replace(' ', '')}",
        f"#VelocityKannada #KannadaPhrases #SpeakKannada"
    ])
...
def upload_to_youtube(video_path, title, description, tags=None, category_id='22'):
    ...
    if tags is None:
        tags = ['education', 'language learning', 'kannada', 'funny']
...
def main():
    ...
    title = "Learn Kannada Daily"
    description = "#shorts #learnkannada #kannada #language #education"
    tags = ['kannada', 'education', 'language learning']
